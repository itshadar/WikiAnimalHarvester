...
class TestWikiAnimalsHarvester:
    ...
    async def test_process_wiki_page_queue_size(
        self, mock_wiki_animals_harvester, mock_parsed_animals
    ):
        # run method
        await mock_wiki_animals_harvester.process_animals_wiki_page()

        # assert that the queue populate as expected
        assert mock_wiki_animals_harvester.page_queue.qsize() == len(
            mock_parsed_animals
        )
    ...
    async def test_process_wiki_page(
        ...
        assert (
            excepted_collateral_adjectives_groups
            == mock_wiki_animals_harvester.collateral_adjectives_groups
        )
