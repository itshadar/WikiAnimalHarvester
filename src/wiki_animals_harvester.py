...
class WikiAnimalsHarvester:
    ...
    def __init__(
        self,
        concurrency: int,
        content_parser: AnimalsPageWikiHTMLParser,
    ...
    def get_content_parser(cls, html_content: str):
        return AnimalsPageWikiHTMLParser.create(
            html_content=html_content, resource_url=cls.RESOURCE_URL
        )
    ...
    async def process_animals_wiki_page(self):
        ...
        for animal_info in self.content_parser.parse_animal_table():
            for collateral_adjective in animal_info.collateral_adjectives:
                self.collateral_adjectives_groups[collateral_adjective].append(
                    animal_info.name
                )
    ...
    async def _activate_harvester_consumers(self):
        consumers = [
            create_task(
                self.page_content_extractor.extract_animal_html_page_image_url_consumer(
                    self.page_queue, self.image_queue
                )
            )
            for _ in range(self.concurrency)
        ]
        consumers.extend(
            create_task(self.image_downloader.download_image_consumer(self.image_queue))
            for _ in range(self.concurrency)
        )
